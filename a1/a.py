...
class Peer:
    ...
    def validate_block(self, block, t):
        ...
        for par in self.blockchain:
            if par.id == block.pid: # Parent found
                ...
                block.block_eyes = par.block_eyes
    ...
    def block_creation(self, I):
        # Simulate Block Creation
        global BlkId
        # print(f"Peer {self.id} creating block {BlkId}")
        self.block_wait += int(np.random.exponential(I/self.hk)) + 1 # Tk update
        block = Block(BlkId)
        BlkId += 1
        block.block_eyes = self.longest_holder.block_eyes
        ...
        for txn in self.transactions:
            ...
            if txns == 1023: # 1 + 1022 + 1(coinbase)
                break
    ...
    def ender(self):
        ...
        for block in self.blockchain:
            f.write(f"Time: {block.time}, ID: {block.id}, PID: {block.pid}\n")
...
class Simulator:
    def __init__(self, args):
        ...
        self.runs = 10000000 # Total milliseconds to run
        ...
        while not connected:
            ...
            for i in range(self.n):
                ...
                for _ in range(connections):
                    ...
                    self.peers[connections].connections.add(i)
            ...
            connected =  (len(visited) == self.n)
            

    def run(self):
        # Initial Block creation
        for peer in self.peers:
            peer.block_creation(self.I)
        ...
        while True:
            ...
            for peer in self.peers:
                ...
                if self.time == peer.block_wait:
                    ...
                    peer.block_creation(self.I) # Create new block and wait for maturity
                    if new_block:
            ...
            while len(block_queue) > 0:
                ...
                self.peers[item[1]].block_creation(self.I) # Continue with block creation
                if valid:
